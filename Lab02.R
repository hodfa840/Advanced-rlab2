...
sheldon_game <- function(player1 , player2) {
  ...
  else if((any((alt1 + c(1, 3)) == 5)) & (alt2 == 5)){
    return("player 1 wins!!")
  }
  
  else if(any((alt1 + c(1, 3)) %% 5 == alt2 ))
  {
    return("Player 1 wins!")
  }
  else {
...
#########################################
my_moving_median <- function(x, n, flag ){
  
  medVec = c()
  for(i in length(x - n)) {
    medVec [i] = median(x[i : i+n], na.rm = flag)
  }
}
return(medVec)
